...
Commander procedury/jídla jsou samostatná ActivityKit vrstva. `CommanderProcedureLiveActivityCoordinator` připravuje nejvýše dvě nejbližší neskončené události. Budoucí aktivita používá ActivityKit scheduled start přes `start: leaveAt`; od `leaveAt` do `startAt` zobrazuje `Začíná za`, od `startAt` `Právě…` a v `endAt` se přes explicitní `TimelineView` přepne na embedded další událost nebo skončený stav. `staleDate = endAt` zůstává pouze stavem zastarání, proto coordinator při dalším reconciliation ukončuje prošlé nebo neplatné aktivity explicitně. AlarmKit Stop intent **nikdy** Commander aktivitu nevytváří ani nemění.

Budoucí lokální ActivityKit start vyžaduje `AlertConfiguration`; implementace používá tichý bundled zvuk `CommanderSilentAlert.wav`, aby nevytvářela druhý slyšitelný alarm. Systém přesto může start alert vizuálně prezentovat. Bez APNs není garantována libovolná přesná lokální update/end mutace v době, kdy aplikace nemá execution time; canonical Schedule a AlarmKit proto zůstávají nezávislým zdrojem funkčního odchodu.
