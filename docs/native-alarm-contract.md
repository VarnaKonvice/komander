...
Canonical native payload nadále obsahuje všechny události. `AlarmSyncService` vytváří nové alarmy pouze pro `leaveAt > now`; `leaveAt == now` se již neplánuje. Čas automatické projekce se zachytí po přijetí rozpisu, nikoli před čekáním na síť. Chybějící minulý platformní alarm se odstraní z lokálního mapování bez opětovného vytvoření.

Výjimkou z úklidu minulých alarmů je **skutečně `.alerting` alarm** se stejným úplným canonical obsahem a dosud neskončenou událostí. Foreground není tlačítko Stop: tento alarm i jeho mapování zůstávají do zastavení, změny/odstranění události nebo jejího konce. U takového alarmu se nevyžaduje již neexistující countdown fireDate. `lastSuccessfulPayload` obsahuje budoucí desired set, zatímco `records` mohou navíc obsahovat tuto omezenou množinu zvonících alarmů. Tento filtr nemění `Schedule`, Live Card ani celý `WatchScheduleSnapshot`.

`ManagedAlarmRecord.presentationContext` chrání pouze závislosti, které nejsou součástí vlastního `NativeAlarm`, ale skutečně mění AlarmKit prezentaci: délku countdown window od konce předchozí události a typ aktuální události pro vizuální klasifikaci. Změna následující události už předchozí alarm neobnovuje; změna předchozího konce ano, pokud změní countdown window. Samotná změna scheduleVersion update nevytváří. Starší záznam bez kontextu projde jednou ověřenou migrací. Selhání cancel vždy ponechá dosavadní mapování pro bezpečný další pokus a zablokuje vytvoření náhrady; neověřený cleanup nesmí předstírat úspěch.


Commander procedury/jídla jsou samostatná ActivityKit vrstva, ale aplikace udržuje **nejvýše jednu vlastní Commander Live Activity**. První úspěšný AlarmKit Stop ji vytvoří; každý další Stop pouze aktualizuje její `ContentState`. Dynamický stav obsahuje krátkou chronologickou frontu nejbližších událostí a `staleDate` se posouvá na konec poslední známé položky. Tím nevznikají konkurenční Commander karty ani závislost na `relevanceScore`.

Stejná Live Activity se časově přepíná přes explicitní `TimelineView`: před `startAt` zobrazuje `Začíná za`, během události `Právě…` a po jejím `endAt` přejde na další známou položku nebo skončený stav. Při překryvu zůstává nahoře dříve zahájená stále probíhající událost a následující položka je zobrazena pod ní; po konci první se druhá automaticky stane hlavní. Foreground reconciliation rozšíří obsah téže aktivity z canonical rozpisu a současně ukončí případné duplicitní staré Commander instance. Bez APNs není garantováno přesné fyzické odstranění karty v libovolném budoucím okamžiku, ale přechody mezi známými časy jsou lokálně odvozené ze stejného canonical Schedule.
...
Watch notifikační zápisy vlastní `WatchCommanderModel`, cache callback pouze invaliduje widget. Přijetí stejného snapshotu také opakuje případný neúspěšný zápis. Celá operace v `WatchLocalNotificationService` prochází FIFO frontou včetně všech await; actor sám tuto serializaci nezaručuje. Read-back porovnává skutečný trigger, titul, tělo a přítomnost zvuku, ne pouze `userInfo`. Neověřený zápis neaktualizuje poslední úspěšnou verzi a chyba zůstane viditelná.

Podrobný [registr invariantů a stabilizační audit](native-stabilization-v2.md) odděluje behaviorální testy jádra, integrační buildy, strukturální kontroly zdrojů a fyzické ověření.
