...
`ManagedAlarmRecord.presentationContext` chrání závislosti mimo vlastní `NativeAlarm`: countdown ownership, typové ikony a následující událost v Stop intentu. Změna souseda obnoví závislý alarm, samotná změna scheduleVersion nikoli. Starší záznam bez kontextu projde jednou ověřenou migrací. Selhání cancel vždy ponechá dosavadní mapování pro bezpečný další pokus a zablokuje vytvoření náhrady; neověřený cleanup nesmí předstírat úspěch.
