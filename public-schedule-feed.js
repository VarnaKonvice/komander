(function(){
  ...
  function persistSchedule(schedule, source){ localStorage.setItem(STORE, JSON.stringify({ source: source || 'public-feed', schedule: schedule })); var legacy = JSON.stringify(toLegacySchedule(schedule)); localStorage.setItem(LEGACY_STORE, legacy); localStorage.setItem(LEGACY_MIRROR_STORE, legacy); window.dispatchEvent(new Event('lazensky-schedule-change')); return schedule; }
  function currentSchedule(){ var stored = safeParse(localStorage.getItem(STORE) || 'null'); if(stored && stored.schedule){ try { return normalizeSchedule(stored.schedule, { defaultVersion: 0 }); } catch(error) {} } var legacy = safeParse(localStorage.getItem(LEGACY_STORE) || 'null'); if(legacy && Array.isArray(legacy.items)) return normalizeSchedule(legacy, { defaultVersion: 0 }); return null; }
  ...
  function calendarContract(schedule){ schedule = schedule || currentSchedule(); return schedule ? schedule.events.map(function(event){ return { stableId: event.stableId, title: event.title, start: event.date+'T'+event.start+':00', end: event.date+'T'+event.end+':00', location: event.location, kind: event.kind, procedureType: event.procedureType || null, mealType: event.mealType || null, leadTimeMinutes: effectiveLeadTime(event, schedule) }; }) : []; }
  ...
  window.LazenskySchedule = { normalizeSchedule: normalizeSchedule, validateSchedule: validateSchedule, getSchedule: currentSchedule, getEffectiveLeadTime: effectiveLeadTime, calendarContract: calendarContract, renderNotificationSettings: notificationSettingsHtml, bindNotificationSettings: bindNotificationSettings, refreshPublicSchedule: refreshPublicSchedule, scheduleUrl: SCHEDULE_URL };
