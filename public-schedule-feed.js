(function(){
  ...
  function effectiveLeadTime(event, schedule, overrides){ schedule = schedule || currentSchedule(); if(!schedule) return 20; var local = arguments.length < 3 ? getLocalSettings() : normalizeLeadTimeOverrides(overrides), type = overrideKey(event.kind === 'meal' ? event.mealType : event.procedureType), automatic = event.kind === 'meal' ? schedule.settings.mealOverrides[type] : schedule.settings.procedureTypeOverrides[type], localType = event.kind === 'meal' ? local.mealOverrides[type] : local.procedureTypeOverrides[type]; if(local.eventOverrides[event.stableId] != null) return local.eventOverrides[event.stableId]; if(localType != null) return localType; if(local.defaultLeadTimeMinutes != null) return local.defaultLeadTimeMinutes; if(event.leadTimeMinutes != null) return event.leadTimeMinutes; return automatic != null ? automatic : schedule.settings.defaultLeadTimeMinutes; }
  ...
  function computeLiveState(scheduleInput, nowInput, overrides){
    ...
    var schedule, effectiveOverrides = arguments.length > 2 ? overrides : getLocalSettings();
