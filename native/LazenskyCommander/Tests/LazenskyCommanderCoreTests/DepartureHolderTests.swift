...
@Test func holderIsVisibleAndStopUsesExistingActivityWithoutForegroundRequests() throws {
  ...
  #expect(live.contains("CommanderUpcomingHero(startAt: bridgeStartAt, accent: currentAccent)"))
  #expect(live.contains("supplementalActivityFamilies([.small])"))
  #expect(live.contains("countingDownIn: Date.distantPast..<startAt, showsHours: true, maxFieldCount: 2"))
