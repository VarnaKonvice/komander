...
@Test func physicalAppHasNoProductionModelNetworkPreferencesOrWatchEntryPoint() throws {
  ...
  #expect(adapter.contains("maximumCommanderActivities = 2"))
  ...
  #expect(adapter.contains("dismissalPolicy: .after(startDate)"))
  ...
  #expect(stopIntent.contains("await holder.end("))
  ...
  #expect(live.contains("if context.state.isDepartureStandby"))
  #expect(live.contains("let bridgeTitle = context.state.nextTitle ?? context.attributes.title"))
